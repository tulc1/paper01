...
class Data(object):
    def __init__(self, num_two_order, dataset, norm_adj, seed, test_ratio):
        ...
        # self.adj_train, self.user_item = self.generate_adj()
        self.adj_train, user_item = self.generate_adj()
        # 增加 计算二阶
        # self.user_item = torch.tensor(self.user_item, dtype=torch.float32)  # .to(torch.device('cuda'))
        user_item = torch.tensor(user_item, dtype=torch.float32)
        # user_item = torch.tensor(user_item, dtype=torch.bool)
        self.indices = torch.nonzero(user_item)
        user_item_two_order = self.create_user_item_two_order(user_item)
        self.num_two_order = num_two_order

        # pair set
        self.user_item_two_order_pairs_set = {idx: np.nonzero(user_item_two_order[idx])[0] + self.num_users
                                              for idx in range(len(user_item_two_order))}
        self.user_item_pairs_set = {idx: np.nonzero(user_item[idx])[0] + self.num_users
    ...
    def create_user_item_two_order(self, user_item):
        print("产生二阶关系")
        user_item = user_item.to(default_device())
        # user_user = torch.matmul(user_item, user_item.T)
        # user_item_two_order = torch.matmul(user_user, user_item)
        item_item = torch.matmul(user_item.T, user_item)
        user_item_two_order = torch.matmul(user_item, item_item)

        user_item_two_order[user_item_two_order > 0] = 1.
        user_item_two_order = user_item_two_order
        user_item_two_order -= user_item  # 去除一阶

        print("二阶关系计算结束")
        return user_item_two_order.detach().cpu().numpy().astype(int)

    def create_item_item(self, user_item):
        print("创建item-item邻接矩阵")
        item_item = torch.matmul(user_item.T, user_item)
        item_item[item_item > 1] = 1
        return item_item.numpy().astype(int)
